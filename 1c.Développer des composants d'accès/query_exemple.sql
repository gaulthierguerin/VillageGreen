-- Active: 1699968333576@@127.0.0.1@3306@exemple
...
SELECT
    ...
    salaire,
    tauxcom,
    salaire * tauxcom - salaire AS commission
FROM employe
WHERE tauxcom IS NOT NULL
ORDER BY tauxcom ASC;
...
SELECT
    CONCAT(nom, " ", prenom) AS `Nom et Prénom des employés`
FROM employe;

--27.Afficher les 5 premières lettres du nom des employés.

SELECT
    SUBSTRING(nom, -5, 5) AS '5 premières lettres nom'
FROM employe;
...
SELECT
    nom,
    LENGTH(CONCAT(nom, prenom)) AS 'Nombre de lettres'
FROM employe;
...
WHERE (
        SELECT
            COUNT(titre) AS 'nombre de titre'
        from employe
    )
GROUP BY titre;
