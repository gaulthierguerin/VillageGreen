...
SELECT nom, noemp, nodep, titre FROM employe
...
/*12.Afficher le nom, et le salaire des employés dont le salaire est compris
entre 20000 et 30000.*/

SELECT nom, salaire FROM employe
WHERE salaire BETWEEN 20000 AND 30000

--13.Afficher le nom des employés commençant par la lettre « H ».

SELECT nom FROM employe
WHERE nom LIKE 'H%'

--14.Afficher le nom des employés se terminant par la lettre « n ».

SELECT nom FROM employe
WHERE nom LIKE '%n'

--15.Afficher le nom des employés contenant la lettre « u » en 3ème position.

SELECT nom FROM employe
WHERE nom LIKE '__u%' 

/*16.Afficher le salaire et le nom des employés du service 41 classés par
salaire croissant*/
...
/*17.Afficher le salaire et le nom des employés du service 41 classés par
salaire décroissant.*/
...
/*18.Afficher le titre, le salaire et le nom des employés classés par titre
croissant et par salaire décroissant.*/

SELECT titre, salaire, nom FROM employe
ORDER BY titre ASC, salaire DESC

/*19.Afficher le taux de commission, le salaire et le nom des employés
...
/*20.Afficher le nom, le salaire, le taux de commission et le titre des
...
/*21.Afficher le nom, le salaire, le taux de commission et le titre des
employés dont le taux de commission est renseigné.*/

SELECT nom, salaire, tauxcom, titre FROM employe
WHERE tauxcom IS NOT NULL

/*22.Afficher le nom, le salaire, le taux de commission, le titre des
...
/*23.Afficher le nom, le salaire, le taux de commission, le titre des
...
/*24.Afficher le nom, le salaire, le taux de commission et la commission des
...
SELECT nom, salaire, tauxcom, salaire*tauxcom AS SalaireTot FROM employe

/*25.Afficher le nom, le salaire, le taux de commission, la commission des
employés dont le taux de commission n'est pas nul, classé par taux de
commission croissant.*/

SELECT nom, salaire, tauxcom, salaire*tauxcom-salaire AS commission FROM employe
WHERE tauxcom IS NOT NULL
ORDER BY tauxcom ASC

/*26.Afficher le nom et le prénom (concaténés) des employés. Renommer
les colonnes*/

SELECT CONCAT(nom," ", prenom) AS `Nom et Prénom des employés` FROM employe

--27.Afficher les 5 premières lettres du nom des employés.

SELECT SUBSTRING(nom, -5, 5) AS '5 premières lettres nom' FROM employe

/*28.Afficher le nom et le rang de la lettre « r » dans le nom des
employés.*/

SELECT nom, titre FROM employe
WHERE 

/*29.Afficher le nom, le nom en majuscule et le nom en minuscule de
l'employé dont le nom est Vrante.*/

SELECT nom, UPPER(nom), LOWER(nom) FROM employe
WHERE nom = 'Vrante'

--30.Afficher le nom et le nombre de caractères du nom des employés.

SELECT nom, LENGTH(CONCAT(nom, prenom)) AS 'Nombre de lettres' FROM employe


--Exemple Set d'exercice 2
